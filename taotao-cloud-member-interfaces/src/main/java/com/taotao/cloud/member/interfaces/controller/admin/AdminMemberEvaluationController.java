...
@Validated
@RestController
@RequestMapping("/member/admin/membe/evaluation")
@Tag(name = "管理端-会员商品评价管理API", description = "管理端-会员商品评价管理API")
public class AdminMemberEvaluationController extends BusinessController {
    ...
    // @RequestLogger
    // @PreAuthorize("@el.check('admin','timing:list')")
    // public Result<PageResult<MemberEvaluationListVO>> queryPage(EvaluationPageQuery
    // evaluationPageQuery) {
    ...
    //        @Parameter(description = "显示状态,OPEN 正常 ,CLOSE 关闭", required = true) @NotNull String
    // status) {
